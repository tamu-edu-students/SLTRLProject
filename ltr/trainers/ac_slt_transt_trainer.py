    ...
    def cycle_dataset(self, loader):
        ...
        for i, data in enumerate(loader, 1):
            ...
            while cursor < num_seq:
                ...
                model_inputs['template_images'] = explore_result['template_images'][cursor:cursor + bs].cuda()
                model_inputs['search_images'] = explore_result['search_images'][:, cursor:cursor + bs].cuda()
                model_inputs['search_anno'] = explore_result['search_anno'][:, cursor:cursor + bs].cuda()
                model_inputs['action_tensor'] = explore_result['action_tensor'][:, cursor:cursor + bs].cuda()
                model_inputs['reward_tensor'] = reward_tensor[:, cursor:cursor + bs].cuda()
            ...
            if self.settings.clip_grad:
                # clippong the normals if set so.
                ...
                total_norm = 0
                # RMS distance in the normalization
                for p in parameters:
