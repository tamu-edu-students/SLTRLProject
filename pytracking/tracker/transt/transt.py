...
class TransT(SiameseTracker):
    ...
    def initialize(self, image, info: dict) -> dict: # image: ndarray: (360, 640, 3)
        hanning = np.hanning(32)
        window = np.outer(hanning, hanning)
        self.window = window.flatten() # ndarray:(1024,)
        ...
        z_crop = self.get_subwindow(image, self.center_pos,
                                    ...
                                    s_z, self.channel_average) # Tensor: (1,3,128,128)
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        outputs = self.net.track(x_crop)    # outputs: {'pred_logits': Tensor(1,1024,2),'pred_boxes': Tensor(1,1024,4)}
        score = self._convert_score(outputs['pred_logits']) # Tensor(1024)
        pred_bbox = self._convert_bbox(outputs['pred_boxes']) # Tensor(4,1024)
